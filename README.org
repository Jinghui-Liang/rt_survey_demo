...
** Launching your platform on server

Open =.env= with a editor you like and change the config to the corresponding values you have on the server. 
For Emacs user, use =C-c C-v C-t= to enable changes with this block. Then call this R function 

#+CALL: create-db-conf()

by =C-c C-c=. This R function generate a conf file to make sure that all the ports and names match.
